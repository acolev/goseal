...
func TestSealOpenRoundTrip(t *testing.T) {
	...
	token, err := Encrypt(kp.Pub, plaintext, aad, kid, aadHint)
	if err != nil {
	...
	decrypted, err := Decrypt(kp.Priv, token, aad)
	if err != nil {
...
func TestOpenFailures(t *testing.T) {
	...
	aad := []byte("aad")
	token, err := Encrypt(kp.Pub, plaintext, aad, "kid", "")
	if err != nil {
		t.Fatal(err)
	}

	// Test wrong private key
	if _, err := Decrypt(otherKP.Priv, token, aad); err == nil {
		t.Error("expected error with wrong private key, got nil")
	}

	// Test wrong AAD
	if _, err := Decrypt(kp.Priv, token, []byte("wrong")); err == nil {
		t.Error("expected error with wrong AAD, got nil")
	}

	// Test malformed token
	if _, err := Decrypt(kp.Priv, "bad.token", aad); err == nil {
